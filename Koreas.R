...
railroads_10 <- ne_download(scale = 10, type = "railroads", category = "cultural", returnclass = "sf")%>%
  st_intersection(bbox)
rivers_10 <- ne_download(scale = 10, type = "rivers_lake_centerlines", category = "physical", returnclass = "sf")%>%
  filter(featurecla == "River")%>%
  st_intersection(bbox)
lakes_10 <- ne_download(scale = 10, type = "lakes", category = "physical", returnclass = "sf")%>%
  st_intersection(bbox)

regions_10 <- ne_download(scale = 10, type = "geography_regions_polys", category = "physical", returnclass = "sf")%>%
  st_intersection(bbox)%>%
  filter(SCALERANK > 1)

disputed_area_10 <- ne_download(scale = 10, type = "admin_0_disputed_areas", category = "cultural", returnclass = "sf")%>%
  st_intersection(bbox)
...
# c2 <- c("#9e391a", "#c4ac7c", "#5e6a58", "#354536","#2d433a")
# c2 <- c("#eae9da", "#e0dccf", "#d1c9bd", "#c6bab5","#bdaea8")
# c2 <- c("#93c293", "#508651", "#f6e0b5", "#E6C6A1", "#d6a165", "#392a11")
c3 <- grDevices::colorRampPalette(c("#d11141", "#00b159", "#00aedb", "#f37735", "#00db9b", "#9b00db"))(10)
swatchplot(c2)
swatchplot(c3)
texture <- grDevices::colorRampPalette(c2, bias = 0.6)(500)
swatchplot(texture)
...
raster_to_matrix(country_elevation_ref)%>%
  ...
  add_shadow(ray_shade(raster_to_matrix(country_elevation_ref),zscale=40)) %>%
  add_shadow(cloud_shade(raster_to_matrix(country_elevation_ref),zscale=40, cloud_cover = 0.4))%>%
  ...
  add_overlay(generate_polygon_overlay(urban_areas_10
                                       ...
                                       , linewidth=NA)
  , rescale_original = TRUE)  %>%
  # add_overlay(generate_polygon_overlay(regions_10
  #                                      , attr(country_elevation_ref,"extent")
  #                                      , heightmap = raster_to_matrix(country_elevation_ref)
  ...
  #                                      , linewidth=NA)
  #             , rescale_original = TRUE)  %>%
  # add_overlay(generate_polygon_overlay(disputed_area_10
  #                                      , attr(country_elevation_ref,"extent")
  #                                      , heightmap = raster_to_matrix(country_elevation_ref)
  #                                      , data_column_fill = "NAME"
  #                                      , palette = "red3")
  #             , rescale_original = TRUE7)  %>%
  # add_water(detect_water(raster_to_matrix(country_elevation_ref), min_area = 400),color="#93b8e2") %>%
# regions_10, disputed_area_10, urban_areas_10
  plot_3d(heightmap = raster_to_matrix(country_elevation_ref)
          , zscale = 40
          , solid = T
          , shadow = T
          , shadowdepth = -1524.000000
          , water = TRUE
          , watercolor = "#93b8e2"
          , zoom = 0.65
          , multicore=TRUE
...
# render_clouds(raster_to_matrix(country_elevation_ref), zscale=100, cloud_cover = 0.4, clear_clouds = T)  


## Render Camera ----
render_depth(heightmap = raster_to_matrix(country_elevation_ref)
             , zscale= 40
             , focallength=300
             , aberration = 0.3
             , vignette = TRUE
...
outfile <- "images/test_plot_koreas_stylized.png"
